...
const filteredNowPlaying = computed(() => {
  return dataPopularTv.value.filter((movie) => movie.id !== parseInt(props.id));
});
...
const selectMovie = async (movie) => {
  try {
    ...
    router.push(`/series/popular/${encodeURIComponent(movie.title)}/${movie.id}`);
  } catch (error) {
    console.error(error);
  }
};
      ...
      <div class="carousel carousel-center w-full p-4 space-x-4 bg-transparent rounded-box wrapper">
        <div class="carousel-item" v-for="(movie, index) in filteredNowPlaying" :key="index">
          <template v-if="movie.selected">
            <RouterLink :to="`/series/popular/${encodeURIComponent(movie.title)}/${movie.id}`">
