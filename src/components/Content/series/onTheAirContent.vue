...
const store = useonTheAirStore();

const dataOnTheAir = computed(() => store.getDataOnTheAir);
...
<template>
  <div class="bg-black py-20">
    ...
    <h1 class="text-white text-2xl font-JakartaSans font-bold text-center">On The Air List</h1>

    <div class="style py-4">
      <div class="flex flex-wrap justify-center items-center gap-4">
        <div class="card w-96 bg-base-100 shadow-xl" v-for="(movie, index) in dataOnTheAir" :key="index">
          <div v-if="itsLoading">
            <Loading />
          </div>

          <RouterLink v-else :to="`/series/ontheair/${encodeURIComponent(movie.original_name)}/${movie.id}`">
