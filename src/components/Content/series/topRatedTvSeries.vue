...
const dataTopRatedTv = computed(() => store.getTopRatedTv);
...
onBeforeMount(async () => {
  ...
  await store.fetchDataTopRatedTv();
...
<template>
  <div class="bg-black py-20">
    ...
    <h1 class="text-white text-2xl font-JakartaSans font-bold text-center">Top Rated List</h1>

    <div class="style py-4">
      <div class="flex flex-wrap justify-center items-center gap-4">
        <div class="card w-96 bg-base-100 shadow-xl" v-for="(movie, index) in dataTopRatedTv" :key="index">
          <div v-if="itsLoading">
            <Loading />
          </div>

          <RouterLink v-else :to="`/series/toprated/${encodeURIComponent(movie.original_name)}/${movie.id}`">
