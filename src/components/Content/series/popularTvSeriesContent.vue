...
const store = usepopularTvStore();

const dataPopularTv = computed(() => store.getPopularTv);
...
onBeforeMount(async () => {
  ...
  await store.fetchDataPopularTv();
...
<template>
  <div class="bg-black py-20">
    ...
    <h1 class="text-white text-2xl font-JakartaSans font-bold text-center">Popular TV List</h1>

    <div class="style py-4">
      <div class="flex flex-wrap justify-center items-center gap-4">
        <div class="card w-96 bg-base-100 shadow-xl" v-for="(movie, index) in dataPopularTv" :key="index">
          <div v-if="itsLoading">
            <Loading />
          </div>

          <RouterLink v-else :to="`/series/popular/${encodeURIComponent(movie.original_name)}/${movie.id}`">
